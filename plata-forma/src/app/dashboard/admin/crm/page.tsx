...
export default function CRMPage() {
    ...
    const formatPhoneForWhatsApp = (phone?: string) => {
        ...
        // Se já tiver 12 ou 13 dígitos, assume que já tem DDI. 
        // Se tiver 10 ou 11, é Brasil sem DDI, então adiciona 55.
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className={`divide-y ${isDark ? 'divide-white/5' : 'divide-gray-100'}`}>
                                        {sortedUsers.map((user) => {
                                            ...
                                            const phoneNum = formatPhoneForWhatsApp(user.phone);
                                            return (
                                                <tr
                                                    key={user.id}
                                                    ...
                                                    <td className="px-6 py-4 text-right">
                                                        <div className="flex items-center justify-end gap-2">
                                                            {health.upsell && (
                                                             ...
                                                             <a
                                                                href={user.phone ? `https://wa.me/${formatPhoneForWhatsApp(user.phone)}` : '#'}
                                                                target={user.phone ? "_blank" : "_self"}
                                                                rel="noreferrer"
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    if (!user.phone) e.preventDefault();
                                                                }}
                                                                className={`p-2 rounded-xl transition-all font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 ${user.phone ? 'bg-[#25D366] hover:bg-[#20ba59] text-white shadow-lg shadow-green-500/20' : isDark ? 'bg-white/5 text-gray-500 cursor-not-allowed border outline-none' : 'bg-gray-100 text-gray-400 cursor-not-allowed border outline-none'}`}
                                                                title={user.phone ? "Abrir no WhatsApp" : "Usuário sem telefone cadastrado"}
                                                            >
                                                                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                                                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.414 0 .018 5.394 0 12.03c0 2.12.551 4.189 1.595 6.046L0 24l6.105-1.602a11.83 11.83 0 005.937 1.598h.005c6.637 0 12.032-5.395 12.034-12.03.002-3.218-1.248-6.242-4.526-8.527z\" />
                                                                </svg>
                                                            </a>
                                                            <button
                                                                onClick={(e) => {
